...
class Ray {
public:
    Ray(const vec3& start, const vec3& end) : mStart(start), mEnd(end), mDist(1.0f) {}
    ...
    inline bool Intersects(const vec3& A, const vec3& B, const vec3& C) {
        return TestIntersection::RayTriangle(A, B, C, *this);
    }
