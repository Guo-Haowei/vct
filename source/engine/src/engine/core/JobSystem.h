...
class Context {
public:
    void AddTaskCount(int num) { mTaskCount.fetch_add(num); }

    bool IsBusy() const { return mTaskCount.load() > 0; }
...
#define JS_PARALLEL_FOR(CTX, INDEX, COUNT, SUBCOUNT, BODY)                              \
    CTX.Dispatch(static_cast<uint32_t>(COUNT), SUBCOUNT, [&](jobsystem::JobArgs args) { \
