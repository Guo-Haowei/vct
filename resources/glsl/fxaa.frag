...
void main() {
    ...
    const float lumaDown  = rgb2luma( texture( FinalImage, uv + inverseScreenSize * vec2( 0, -1 ) ).rgb );
    const float lumaUp    = rgb2luma( texture( FinalImage, uv + inverseScreenSize * vec2( 0, 1 ) ).rgb );
    const float lumaLeft  = rgb2luma( texture( FinalImage, uv + inverseScreenSize * vec2( -1, 0 ) ).rgb );
    const float lumaRight = rgb2luma( texture( FinalImage, uv + inverseScreenSize * vec2( 1, 0 ) ).rgb );
    ...
    const float lumaDownLeft  = rgb2luma( texture( FinalImage, uv + inverseScreenSize * vec2( -1, -1 ) ).rgb );
    const float lumaUpRight   = rgb2luma( texture( FinalImage, uv + inverseScreenSize * vec2( 1, 1 ) ).rgb );
    const float lumaUpLeft    = rgb2luma( texture( FinalImage, uv + inverseScreenSize * vec2( -1, 1 ) ).rgb );
    const float lumaDownRight = rgb2luma( texture( FinalImage, uv + inverseScreenSize * vec2( 1, -1 ) ).rgb );
    ...
    float lumaEnd1 = rgb2luma( texture( FinalImage, uv1 ).rgb );
    float lumaEnd2 = rgb2luma( texture( FinalImage, uv2 ).rgb );
    ...
    if ( !reachedBoth )
    {
        ...
        for ( int i = 2; i < ITERATIONS; i++ )
        {
            ...
            if ( !reached1 )
            {
                lumaEnd1 = rgb2luma( texture( FinalImage, uv1 ).rgb );
                lumaEnd1 = lumaEnd1 - lumaLocalAverage;
            }
            // If needed, read luma in opposite direction, compute delta.
            if ( !reached2 )
            {
                lumaEnd2 = rgb2luma( texture( FinalImage, uv2 ).rgb );
    ...
    vec3 finalColor = texture( FinalImage, finalUv ).rgb;
